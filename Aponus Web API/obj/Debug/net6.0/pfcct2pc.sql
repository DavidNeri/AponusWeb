...
INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20230123171353_ModificacionCampoPERFIL_TablaCuantitativosDetalle', N'7.0.1');
GO
...
ALTER TABLE [PRODUCTOS] DROP CONSTRAINT [FK_PRODUCTOS_COMPONENTES_CUANTITATIVOS_ComponentesCuantitativoIdProducto_ComponentesCuantitativoIdComponente];
GO

DROP INDEX [IX_PRODUCTOS_ComponentesCuantitativoIdProducto_ComponentesCuantitativoIdComponente] ON [PRODUCTOS];
GO
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[PRODUCTOS]') AND [c].[name] = N'ComponentesCuantitativoIdComponente');
IF @var8 IS NOT NULL EXEC(N'ALTER TABLE [PRODUCTOS] DROP CONSTRAINT [' + @var8 + '];');
ALTER TABLE [PRODUCTOS] DROP COLUMN [ComponentesCuantitativoIdComponente];
GO
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[PRODUCTOS]') AND [c].[name] = N'ComponentesCuantitativoIdProducto');
IF @var9 IS NOT NULL EXEC(N'ALTER TABLE [PRODUCTOS] DROP CONSTRAINT [' + @var9 + '];');
ALTER TABLE [PRODUCTOS] DROP COLUMN [ComponentesCuantitativoIdProducto];
GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20230125003055_FK_COMPONENTESPESABLES_PRODUCTOS', N'7.0.1');
GO
...
ALTER TABLE [COMPONENTES_CUANTITATIVOS] ADD CONSTRAINT [FK_COMPONENTESCUANTITATIVOS_PRODUCTOS_IDPRODUCTO] FOREIGN KEY ([ID_PRODUCTO]) REFERENCES [PRODUCTOS] ([ID_PRODUCTO]) ON DELETE SET NULL;
GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20230125003213_FK_PRODUCTOS_COMPONENTESPESABLES', N'7.0.1');
GO
